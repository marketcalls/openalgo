...
- Chart an instrument whose name contains a hyphen. Whether `BAJAJ-AUTO` is an instrument or a subtraction is now settled by whether an instrument of that name exists rather than by reading the text, so a box that names one is never treated as arithmetic and never has a symbol appended to it. A hyphenated symbol put into a real expression is quoted, which is the chart grammar's own escape, so `'NSE:BAJAJ-AUTO'/NIFTY` resolves as written.
