...
The most critical class of findings involves **hardcoded cryptographic secrets and fallback keys** (VULN-001, VULN-002, VULN-009) that ship in the open-source repository and are silently used if operators do not generate unique values. Note: `install/install.sh` mitigates VULN-001 and partially mitigates VULN-002 by auto-generating unique `APP_KEY` and `API_KEY_PEPPER` values, reducing severity for server deployments using this script. However, `TELEGRAM_KEY_SALT` is never generated by install.sh, and the `settings_db.py` fallback key (VULN-009) remains unaddressed. Manual deployments that skip install.sh remain fully exposed. The second most impactful class is **plaintext storage of sensitive data** in SQLite databases (VULN-008, VULN-012, VULN-013, VULN-014) -- because SQLite stores the entire database as a single file, theft of one file exposes TOTP secrets, Telegram bot tokens, flow API keys, and Samco credentials without any decryption step.
...
|------|----------|--------|-------|
| VULN-001 | Critical | Mitigated | `install/install.sh` auto-generates unique APP_KEY and API_KEY_PEPPER; only affects manual `cp .sample.env .env` deployments |
...
Severity: Critical (Low when deployed via install/install.sh)
...
Deployment note: `install/install.sh` auto-generates unique values for both `APP_KEY` and `API_KEY_PEPPER` via `secrets.token_hex(32)` (lines 419-420) and substitutes the sample defaults in `.env` (lines 708-709). Server deployments using this script are **not affected**. The risk applies only to manual deployments where users run `cp .sample.env .env` without regenerating secrets.
