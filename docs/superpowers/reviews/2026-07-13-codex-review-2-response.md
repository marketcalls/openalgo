...
| 4 | Fractional/sandbox underestimated | **Confirmed** | `sandbox_db.py:66` `quantity Integer`, `price DECIMAL(10,2)`; `:75` "no partial fills"; `:188` single INR account; `order_manager.py:74` & `fund_manager.py` cast qty to `int`; `chartink_db` integer qty | P5 is a **replatform** (schema + execution), now depends on **P2+P3+P4** |
