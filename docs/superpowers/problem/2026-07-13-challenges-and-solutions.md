...
|---|-------------------|--------------|--------------------|-------|
| 1 | **No *unified full-lifecycle* contract** — REST/auth/data duck-typed via `importlib` (a WS base class exists) | "Plug in your exchange" means reverse-engineering Zerodha; out-of-tree needs a distributable package + registry, not just a loader | A versioned, public **`BrokerAdapter` contract** (required + optional protocols, typed errors, idempotency) + **conformance suite** + **distributable package & unified registry** | P1 |
