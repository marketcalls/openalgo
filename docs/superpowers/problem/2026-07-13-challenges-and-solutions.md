...
| 2 | **Symbol format is Indian-derivative-shaped** (`NIFTY28MAR2420800CE`) | Crypto perps (`BTCUSDT`), US OCC options, forex don't fit the expiry+CE/PE mold | A structured **Instrument** underneath the flat symbol; the string becomes a deterministic **rendering** — Indian renders byte-identical, US reuses the same renderers, crypto adds one | P2 |
