...
- FR-004: The React route set must register before REST and UI blueprints when `frontend/dist` is available. Trace: `DISCOVERY_MAP.md` `Entry Points And Runtime`, `Conditional Surfaces`; BDD: `docs/bdd/auth_and_setup.feature`.
...
- FR-006: The app must wait up to 30 seconds for database initialization before request handling and remove scoped SQLAlchemy sessions during teardown. Trace: `DISCOVERY_MAP.md` `Entry Points And Runtime`; BDD: `docs/bdd/auth_and_setup.feature`.
...
- FR-081: Active app sessions must be device-specific, replace an existing row for the same user and IP, and retain no more than five active-session rows per user. Trace: `database/auth_db.py`; BDD: `docs/bdd/session_lifecycle.feature`.
- FR-082: Session-status polling must refresh `last_seen` no more than once every 30 seconds and report the active-session count. An expired broker token must preserve a valid app session and report `broker_session_expired` so the user can reconnect. Trace: `blueprints/auth.py`, `database/auth_db.py`; BDD: `docs/bdd/session_lifecycle.feature`.
- FR-083: Re-authentication with unchanged broker and feed tokens must detect plaintext equality after decryption and avoid tearing down the shared cross-process market-data feed. Password changes must revoke all active device sessions. Trace: `database/auth_db.py`, `blueprints/auth.py`; BDD: `docs/bdd/session_lifecycle.feature`.
...
- FR-020: RESTX namespaces must be registered under `/api/v1`, while interactive Swagger documentation remains deliberately disabled through `doc=False`. The product must not advertise `/api/docs`. Trace: `restx_api/__init__.py`; BDD: `docs/bdd/rest_api_inventory.feature`.
...
- FR-022: The REST API must expose account, order, market data, option service, analyzer, notification, and utility endpoints listed in the 57-method RESTX inventory. Trace: `DISCOVERY_MAP.md` `RESTX Endpoint Inventory`; BDD: `docs/bdd/rest_api_inventory.feature`.
...
- FR-033: The API surface must include positionbook, openposition, holdings, funds, and margin endpoints under `/api/v1`. Trace: `DISCOVERY_MAP.md` `RESTX Endpoint Inventory`; BDD: `docs/bdd/account_and_depth.feature`, `docs/bdd/rest_api_inventory.feature`.
- FR-034: The web surface must include orderbook, tradebook, positions, holdings, close-position, close-all, cancel-all, cancel-order, and modify-order routes. Trace: `DISCOVERY_MAP.md` `Flask Blueprint Route Inventory`; BDD: `docs/bdd/account_and_depth.feature`, `docs/bdd/orders_and_action_center.feature`.
...
- FR-044: Depth must validate symbols, resolve feed token and broker, and pass broker-specific user ID when supported. REST depth is a snapshot; continuous depth requires a WebSocket `Depth` subscription. Trace: `DISCOVERY_MAP.md` `Market Data And History`; BDD: `docs/bdd/account_and_depth.feature`.
...
- FR-049: The proxy subscriber must bind the configured ZeroMQ endpoint and subscribe to all topics; broker adapter publishers must connect to that endpoint so multiple publishers can feed one proxy. Trace: `websocket_proxy/server.py`, `websocket_proxy/connection_manager.py`; BDD: `docs/bdd/websocket_streaming.feature`.
...
- FR-074: Telegram and WhatsApp subscribers must notify successful order-related events and skip failure and analyzer-error notification. Automatic Telegram event and Flow alerts must also honor the persisted bot `is_active` gate. Trace: `DISCOVERY_MAP.md` `Events, Logs, Notifications, And Observability`; BDD: `docs/bdd/notifications_observability.feature`.
- FR-075: Telegram and WhatsApp RESTX notification APIs must support explicit direct notification requests. Explicit admin and notify operations are not governed by the automatic-alert `is_active` gate. Trace: `DISCOVERY_MAP.md` `RESTX API V1`, `Events, Logs, Notifications, And Observability`; BDD: `docs/bdd/notifications_observability.feature`.
- FR-076: Traffic logging must asynchronously store client IP, method, path, status, duration, host, error, and user ID in `logs.db`, separate from the primary application database. Trace: `utils/traffic_logger.py`, `database/traffic_db.py`; BDD: `docs/bdd/notifications_observability.feature`.
...
### Scalping, Analytics Tools, And Registered Placeholders

- FR-084: The session-authenticated scalping terminal must discover F&O underlyings, expiries, and ATM-centered chains; validate exchange, product, action, quantity, lot multiples, and server-side order rails before execution; and ensure exit operations only reduce existing exposure. Trace: `blueprints/scalping.py`; BDD: `docs/bdd/scalping_and_tools.feature`.
- FR-085: Scalping stop, target, and trailing-stop state must persist per exchange, symbol, product, and live/analyzer mode. A singleton server risk monitor must continue evaluating stored controls after the browser disconnects and submit freeze-safe risk-reducing exits on a breach. Trace: `database/scalping_db.py`, `services/scalping_risk_monitor_service.py`; BDD: `docs/bdd/scalping_and_tools.feature`.
- FR-086: Analytics routes must include futures calendar-spread arbitrage, Gamma Density, and OI Range views derived from current futures or option-chain data. Trace: `blueprints/arbitrage.py`, `blueprints/gamma_density.py`, `frontend/src/pages/OIRange.tsx`; BDD: `docs/bdd/scalping_and_tools.feature`.
- FR-087: The Telegram REST webhook must validate its configured secret and acknowledge a valid update ID, but clients must not assume command dispatch because the current handler does not invoke one. Trace: `restx_api/telegram_bot.py`; BDD: `docs/bdd/notifications_observability.feature`.
- FR-088: Telegram REST broadcast must remain documented as a registered placeholder that reports zero successful and zero failed deliveries until fan-out is implemented. Trace: `restx_api/telegram_bot.py`; BDD: `docs/bdd/notifications_observability.feature`.
- FR-089: The `/pnltracker/legacy` route must not be advertised as usable while it renders the absent `templates/pnltracker.html`; the React `/pnltracker` surface is the supported UI. Trace: `blueprints/pnltracker.py`; conflict: `docs/prd/CONFLICTS.md`.

## Broker Support

Current broker count is 37. Broker support is plugin-based and loaded from `broker/*/plugin.json`. Live GTT modules were found only for Dhan and Zerodha. Trace: `DISCOVERY_MAP.md` `Broker Plugins`; BDD: `docs/bdd/broker_sessions.feature`, `docs/bdd/gtt_orders.feature`.
...
| acagarwalxts | AC Agarwal (XTS) | IN_stock | false | NSE, BSE, NFO, BFO, NSE_INDEX, BSE_INDEX |
...
- Broker-specific response payload shapes are not exhaustively verified for all 37 brokers.
...
- `docs/broker-integration-guide.md`, the code inventory, and `.sample.env` must stay aligned on the current 37 plugin directories.
