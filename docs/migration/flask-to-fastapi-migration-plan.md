...
> **Bottom line up front.** OpenAlgo's *bulk* business logic — most of the service layer, the sandbox/flow/historify engines, the ZeroMQ bus, the websocket proxy, the MCP engine, and the React UI — is framework-agnostic and lifts unchanged. **But the Flask coupling is NOT a clean thin shell** (this corrects the v1 framing; see §1a for the audit): 23 files outside the HTTP layer import Flask — including **11 broker API/mapping files** that use `flask.session` — and **48 files import `extensions.socketio` directly**, including all 36 broker `master_contract_db.py` modules. Session/auth product logic lives in `utils/`. **Therefore a mandatory pre-migration hardening phase on Flask (Phase 1) — request-context abstraction, a notifier abstraction, a single lifecycle owner, and broker decoupling — must precede any FastAPI code**, so that the framework swap itself touches only true shell code. **The single highest-risk item remains the production runtime model: eventlet + Gunicorn `-w 1`, fundamentally incompatible with asyncio.** FastAPI's value proposition is async; adopting it means replacing the runtime *and* the realtime host simultaneously — high-effort, high-blast-radius, in a system that places live trading orders. A **strangler-fig / incremental** approach with the hardening gate is strongly recommended over a big-bang rewrite.
...
| **Broker integrations** | `broker/*/` (36 brokers) | **NOT None** — 11 API/mapping files import `flask.session`; all 36 `master_contract_db.py` import `extensions.socketio` | **Lifts after Phase 1 decoupling** (P1-06/P1-08) |
...
| 3 | Socket.IO is deeply mixed into core logic, not centralized in EventBus | **VALIDATED** — v1's "~18 emit sites, mostly EventBus" was wrong | **48 files** import `extensions.socketio` outside `blueprints/`/`subscribers/` — including **36 broker `database/master_contract_db.py`** (one per broker), 7 services (`orderstatus_service.py:9`, `openposition_service`, `order_router_service`, `historify`, `telegram_bot`, `whatsapp_bot`), `utils/api_analyzer.py`, `utils/auth_utils.py`, `utils/session.py`, `database/master_contract_cache_hook.py:8` |
...
> The *bulk* of business logic (sandbox engine, flow/historify engines, options analytics, websocket proxy, ZMQ, schedulers, MCP engine) is framework-agnostic and lifts unchanged. **But the boundary is not clean at the edges:** 23 files outside the HTTP shell import Flask (including 11 broker API/mapping files), 48 files import `extensions.socketio` directly (including every broker's master-contract module), and session/auth product logic lives in `utils/`. **A pre-migration hardening phase on Flask (Phase 1) is therefore mandatory, not optional** — decouple these via a request-context abstraction (`contextvars`), a notifier abstraction for events, and a single lifecycle owner *before* introducing FastAPI, so the framework swap itself touches only true shell code.
...
> **Corrected governing principle (supersedes the v1 wording, aligned with §1a):** the *engines* — `sandbox/`, `websocket_proxy/`, `mcp/`, and most of `services/` and `database/` — are framework-agnostic and kept byte-for-byte. **But the boundary is not clean: the edges import Flask.** 23 files outside the HTTP shell import Flask (incl. 11 broker files using `flask.session`), **48 files import `extensions.socketio` directly** (incl. all 36 broker `master_contract_db.py` modules — NOT the ~18 v1 claimed), and session/auth product logic lives in `utils/`. So the per-feature tables below show "engine kept as-is" **on the explicit precondition that Phase 1 hardening first severs those edge imports** (notifier abstraction for the 48 emit sites, `flask.session` removal from broker code, context/session seams). After Phase 1, the user-visible behavior (routes, JSON, event names, schedules, webhooks) stays identical and only the HTTP shell is rewritten.
...
**The one genuinely shared migration task across all of the above is SocketIO — and it is bigger than v1 estimated** (corrected in §1a claim 3): **48 files** import `extensions.socketio` directly outside the shell layers, including **all 36 broker `master_contract_db.py` modules**, 7 services, and 3 utils. Order events flow through the framework-agnostic EventBus (`utils/event_bus.py` + `subscribers/`), but master-contract progress, historify jobs, WhatsApp pairing, and order-status emits do **not** — they call `socketio.emit` inline. The fix is the Phase 1 **notifier abstraction**: these modules publish domain events to a notifier interface; a single subscriber owns the actual Socket.IO emit. After that, the framework swap touches one file instead of 48. **Event names and payloads stay identical** (the React client subscribes to fixed strings), so the UI sees no change. The ASGI mount must also keep **long-polling transport** working — the frontend pins `transports: ['polling']` (§1a).
...
- `async_master_contract_download()` runs on a background thread, tracks status transitions in `master_contract_status_db` (`downloading`/`error`/success with duration + per-exchange stats), dynamically imports `broker.{broker}.database.master_contract_db`, and completion fans out to the UI via the `master_contract_download`/`cache_loaded` events (the 36 broker emit sites → notifier abstraction, P1-06) plus strategy restoration via `master_contract_cache_hook`.
...
- **Notifier abstraction for events**: broker/database/service code publishes domain events; one subscriber owns `socketio.emit`. Rewire all **48** direct importers — the 36 broker `master_contract_db.py` modules (mechanical, one pattern), 7 services, `utils/api_analyzer.py`, `database/master_contract_cache_hook.py`.
...
| R11 | Hidden coupling broader than estimated (validated §1a: 23 Flask-importing files incl. 11 broker; 48 socketio importers incl. 36 brokers) | **Confirmed** | High | Phase 1 hardening is mandatory; CI grep gate prevents regression |
