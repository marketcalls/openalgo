...
> **Bottom line up front.** OpenAlgo's *bulk* business logic — most of the service layer, the sandbox/flow/historify engines, the ZeroMQ bus, the websocket proxy, the MCP engine, and the React UI — is framework-agnostic and lifts unchanged. **But the Flask coupling is NOT a clean thin shell** (this corrects the v1 framing; see §1a for the audit): 23 files outside the HTTP layer import Flask — including **11 broker API/mapping files** that use `flask.session` — and **43 engine-scope files import `extensions.socketio` directly (44 including the app shell `app.py`)**, among them all 33 broker `master_contract_db.py` modules. Session/auth product logic lives in `utils/`. **Therefore a mandatory pre-migration hardening phase on Flask (Phase 1) — request-context abstraction, a notifier abstraction, a single lifecycle owner, and broker decoupling — must precede any FastAPI code**, so that the framework swap itself touches only true shell code. **The single highest-risk item remains the production runtime model: eventlet + Gunicorn `-w 1`, fundamentally incompatible with asyncio.** FastAPI's value proposition is async; adopting it means replacing the runtime *and* the realtime host simultaneously — high-effort, high-blast-radius, in a system that places live trading orders. A **strangler-fig / incremental** approach with the hardening gate is strongly recommended over a big-bang rewrite.
...
| **SocketIO (events)** | `extensions.py:6`, `subscribers/socketio_subscriber.py`, **+43 engine-scope importers (44 incl. app shell)** | High — Flask-SocketIO host **and scattered direct `socketio.emit` across core** | **Re-host** → python-socketio ASGI, **after** notifier abstraction collapses 43→1 (P1-05/06/07) |
...
| 3 | Socket.IO is deeply mixed into core logic, not centralized in EventBus | **VALIDATED** — v1's "~18 emit sites, mostly EventBus" was wrong | **Canonical count: 43 engine-scope files** (`services/ database/ broker/ utils/ sandbox/ websocket_proxy/ mcp/ events/`, the P0-09 gate scope) **import `extensions.socketio`; 44 when the app shell `app.py` is included.** Among them: **33 broker `database/master_contract_db.py`** (one per broker), 7 services (`orderstatus_service.py:9`, `openposition_service`, `order_router_service`, `historify`, `telegram_bot`, `whatsapp_bot`), `utils/api_analyzer.py`, `utils/auth_utils.py`, `utils/session.py`, `database/master_contract_cache_hook.py:8` |
...
| 6 | Template usage understated — many `render_template` calls | **VALIDATED** (nuanced) | **44 textual `render_template(` matches, of which 2 are commented out in `blueprints/admin.py` → 42 active calls across 20 blueprint files**, plus 1 live `render_template_string` (`mcp_oauth.py:641`). All 42 reference templates that don't exist (no `templates/` dir) — dead routes shadowed by React, but **each needs an explicit disposition** (delete / convert to JSON / redirect) during the port; they cannot be silently ignored |
...
> The *bulk* of business logic (sandbox engine, flow/historify engines, options analytics, websocket proxy, ZMQ, schedulers, MCP engine) is framework-agnostic and lifts unchanged. **But the boundary is not clean at the edges:** 23 files outside the HTTP shell import Flask (including 11 broker API/mapping files), 43 engine-scope files import `extensions.socketio` directly (44 incl. the app shell; including every broker's master-contract module), and session/auth product logic lives in `utils/`. **A pre-migration hardening phase on Flask (Phase 1) is therefore mandatory, not optional** — decouple these via a request-context abstraction (`contextvars`), a notifier abstraction for events, and a single lifecycle owner *before* introducing FastAPI, so the framework swap itself touches only true shell code.
...
| `Werkzeug` | Starlette equivalents — **but keep installed until two non-middleware usages are replaced**: `werkzeug.utils.secure_filename` (`blueprints/python_strategy.py:37` — file-upload sanitization, needs an equivalent-strength replacement) and `werkzeug.exceptions.Forbidden` (`utils/security_middleware.py:7`, retired with the ASGI middleware rewrite) | WSGI middleware → ASGI; `secure_filename` replacement tracked in P5-03 |
...
- **Route-surface parity goes beyond JSON bodies — Flask and Starlette defaults differ** on exactly the dimensions snapshots tend to miss: allowed **methods** (and the 405 shape), **trailing-slash behavior** (Flask 308-redirects to the canonical form by default; Starlette's `redirect_slashes` behaves differently), **redirect status codes** (301/302/307/308), **`Content-Type` defaults** (incl. charset suffix), **CSV/text download headers** (`Content-Disposition`), **`Set-Cookie` attributes**, and **cache headers** on static/SPA routes. The Phase 0 harness must record all of these per route (P0-14), not just body+status.
- **Deterministic harness design (P0-01)** — many endpoints depend on live brokers, market hours, daily tokens, DB state, and moving quotes; naive snapshots would flake. The harness requires: a **fake/test broker fixture** (a `broker/` plugin implementing the standard module surface with canned responses), **frozen time** (IST-aware, so intraday/expiry/squareoff logic is reproducible), **seeded databases** (fresh SQLite/DuckDB fixtures per run), **network isolation** (hard-fail any real outbound HTTP during contract runs), and **snapshot redaction rules** (normalize timestamps/order-ids/latency fields before diffing).
...
> **Corrected governing principle (supersedes the v1 wording, aligned with §1a):** the *engines* — `sandbox/`, `websocket_proxy/`, `mcp/`, and most of `services/` and `database/` — are framework-agnostic and kept byte-for-byte. **But the boundary is not clean: the edges import Flask.** 23 files outside the HTTP shell import Flask (incl. 11 broker files using `flask.session`), **43 engine-scope files import `extensions.socketio` directly (44 incl. the app shell)** — incl. all 33 broker `master_contract_db.py` modules, NOT the ~18 v1 claimed — and session/auth product logic lives in `utils/`. So the per-feature tables below show "engine kept as-is" **on the explicit precondition that Phase 1 hardening first severs those edge imports** (notifier abstraction for the 43 emit-site files, `flask.session` removal from broker code, context/session seams). After Phase 1, the user-visible behavior (routes, JSON, event names, schedules, webhooks) stays identical and only the HTTP shell is rewritten.
...
Flask-RESTX docs are **disabled** today (`restx_api/__init__.py:10` `doc=False`) — there is no public Swagger UI. FastAPI **enables `/docs`, `/redoc`, and `/openapi.json` by default.** Shipping the migration as-is would **add a new public surface** that exposes the full API schema — a security/parity regression for a single-user self-hosted trading app. **Decision: disable them by default** (`FastAPI(docs_url=None, redoc_url=None, openapi_url=None)`) to match current behavior, OR gate them behind authentication if the project deliberately wants docs. This must be a conscious choice in P2-02 (now in its acceptance criteria), asserted by a Phase 0/2 test that `/docs` returns 404 (or 401) unless explicitly enabled. (Native OpenAPI remains available internally for contract tests even with the public routes off.)

### 3.22 Proxy headers, real client IP & external URL generation
A cluster of behaviors hangs off proxy/header handling and deserves its own parity item (P2-18), because uvicorn's defaults differ from the current WSGI setup:

- **Real client IP** — `TRUST_PROXY_HEADERS` (`.sample.env:337`, default `FALSE`) governs whether `X-Forwarded-For` is honored via `utils/ip_helper.get_real_ip()`. This feeds **IP bans** (security middleware), **rate-limit keys** (Flask-Limiter `get_remote_address` today), and **login-attempt logging**. Under uvicorn the equivalent is `--proxy-headers` + `--forwarded-allow-ips`; the migration must map `TRUST_PROXY_HEADERS` to that configuration so the same env var keeps the same meaning, and assert that a spoofed `X-Forwarded-For` is **ignored when the flag is FALSE** (today's safe default) — getting this wrong flips an IP-ban/rate-limit bypass open.
- **External URL generation** — `HOST_SERVER` (`.sample.env:75`) and Flask's `url_for(..., _external=True)` (used in `blueprints/auth.py` and `brlogin.py`) build **broker OAuth callback URLs, MCP OAuth URLs, and links sent outside the app**. Starlette has no `url_for(_external=True)` equivalent with identical semantics; the port must generate these from `HOST_SERVER`/request base-URL with the same precedence the MCP config documents (`MCP_LOOPBACK_URL > HOST_SERVER > http://127.0.0.1:FLASK_PORT`, `.sample.env:443`). Broker OAuth breaks visibly if a callback URL changes by even a scheme or trailing slash — cover with P0-04/P2-18 tests.
- **Webhook URLs** shown in the UI derive from the same `HOST_SERVER` — assert they render identically.
...
- **Notifier abstraction for events**: broker/database/service code publishes domain events; one subscriber owns `socketio.emit`. Rewire all **43** engine-scope importers — the 33 broker `master_contract_db.py` modules (mechanical, one pattern), 7 services, `utils/api_analyzer.py`, `utils/auth_utils.py`, `utils/session.py`, `database/master_contract_cache_hook.py`. **The notifier must also harden the EventBus lifecycle** (verified gaps): `bus.subscribe()` is a plain `append` with no dedup (`utils/event_bus.py` — the `name` arg is logging-only), so a second `register_all()` call double-registers every subscriber and doubles every emit; and the EventBus `ThreadPoolExecutor` has no `shutdown()` path. Require **idempotent registration** (keyed dedup or a registered-once guard), **duplicate-start protection** under dev-reloader/lifespan re-entry, and an explicit `executor.shutdown(wait=...)` in the lifecycle owner's stop sequence.
- **ContextVar propagation across threads is an explicit rule, not an assumption**: `contextvars` do **not** automatically flow into manually-submitted `ThreadPoolExecutor` tasks (traffic-logger executor, EventBus pool, notifier work, `async_log_*` DB executors). Every `executor.submit()` of work that reads request context must capture it first (`contextvars.copy_context().run(...)` or snapshot the needed values into the task args). Codify this in the P1-01 abstraction's API and test it — otherwise latency/IP/user fields silently go empty under the new runtime.
...
| R11 | Hidden coupling broader than estimated (validated §1a: 23 Flask-importing files incl. 11 broker; 43 engine-scope socketio importers incl. 33 brokers) | **Confirmed** | High | Phase 1 hardening is mandatory; CI grep gate prevents regression |
...
**Proceed only if** the goal is long-term maintainability + native async headroom and the team can fund a multi-phase program with full contract-test coverage. The architecture supports it — the *engines* are decoupled and the market-data path is already asyncio — **but the edges are not clean** (§1a: 23 Flask-importing files, 43 engine-scope socketio importers, session logic in `utils/`), so the pre-migration hardening phase is mandatory, and `fastapi` itself is not yet a dependency.
