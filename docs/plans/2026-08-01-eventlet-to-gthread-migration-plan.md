...
### A13. APScheduler misfire grace

**Correction (rev 6).** Rev 4 claimed `blueprints/python_strategy.py:110` has no `max_instances` or `coalesce`. That is **false** — APScheduler 3.11.2 constructs `{'misfire_grace_time': 1, 'coalesce': True, 'max_instances': 1}` by default, verified against the installed package. Duplicate strategy starts are already prevented by the library.
...
**One request-path limiter sleeps inside its own lock.** `blueprints/pnltracker.py:117-122` holds `self.lock` across `time.sleep()`, so concurrent callers both serialize *and* each hold a worker thread for the full wait. `broker/angel/api/data.py` shows the correct pattern — reserve the slot inside the lock, sleep outside it (`GT-B2-06`).

**Classification (complete, reproducible).** Generated by [`scripts/gthread_sleep_inventory.py`](../../scripts/gthread_sleep_inventory.py), which walks the AST and counts `time.sleep`, `asyncio.sleep` and `eventlet.sleep` call nodes. Re-run it after any broker change; do not hand-count with grep, which both over- and under-counts (rev 4 reported 203/103 from grep against an actual 194/111).

| Category | Sites | Files | Holds a request thread? |
| --- | ---: | ---: | --- |
| **Request path** (`broker/*/api/**`) | **111** | **48** | **Yes — this is the budget term** |
| Streaming threads (`streaming/`, `*websocket*`) | 78 | 34 | No, own threads |
| Background master-contract download | 5 | 3 | No, background thread |
| **Total** | **194** | **85** | |

The eight modules named in rev 2 were the *steady-state throttles*; the real request-path exposure is **111 sites across 48 broker files**. Worst case is bounded by the slowest configured interval on the active broker, not the sum — only one broker is active per deployment. Re-derive when switching brokers.
...
**Applying B1 — corrected in rev 6.** Rev 4 asserted 32 threads had ~30% headroom. That was arithmetically wrong: the long-lived baseline alone is 20 (Socket.IO at 2 threads per polling client) + 5 (strategy SSE) + 5 (MCP SSE) + 2 (loopback) = **32 threads before a single ordinary request, broker sleep, or basket fan-out**. At 32 the pool is 100% consumed by idle streams.

Sizing from the 70% sustained-utilization target:

| Term | Threads |
| --- | ---: |
| Socket.IO (10 clients x 2) | 20 |
| Python Strategy SSE | 5 |
| MCP SSE | 5 |
| Internal loopback reserve | 2 |
| **Long-lived baseline** | **32** |
| Ordinary HTTP concurrency | 6 |
| Broker request-path sleeps (active broker) | 4 |
| Basket fan-out headroom | 4 |
| **Working total** | **46** |
| At 70% sustained utilization | **66** |

**The canary value is therefore 64, not 32.** 32 would deadlock at the stated stream counts. Confirm the §9 product rows before fixing this number — halving the MCP SSE row moves it materially.
...
### 10.4 Applied in rev 6

| Error | Correction |
| --- | --- |
| Tracker CSV was never committed | `.gitignore:54` has `*.csv`, so `git add docs/plans/` silently skipped it and the published plan linked to a file absent from the repo. Force-added, matching how `docs/migration/flask-to-fastapi-migration-tracker.csv` is tracked. |
| Sleep inventory did not add up | Claimed 203 total against a table summing to 183. Replaced hand-grepping with `scripts/gthread_sleep_inventory.py` (AST-based, counts `time`/`asyncio`/`eventlet` sleep call nodes): **194 total, 111 request-path across 48 files**. |
| A13 claimed the strategy scheduler has no `max_instances`/`coalesce` | **False.** APScheduler 3.11.2 defaults to `{'misfire_grace_time': 1, 'coalesce': True, 'max_instances': 1}`. The real exposure is the 1-second misfire grace, which causes *skipped* starts, not duplicated ones. |
| "32 threads has ~30% headroom" | Arithmetically wrong. The long-lived baseline is itself 32 (20 + 5 + 5 + 2), so 32 is 100% consumed before any request. Working total 46; **canary is 64 at the 70% target**. |
| CI and diagnostics scheduled after the cutover | Backwards — they are how a bad cutover is detected. Now PR-9 and PR-10, both shipping on eventlet, before PR-11. |
| Class-level mutable attribute sweep not run | Run in rev 6; 1 new open row, 2 resolved. |
...
**That is three consecutive revisions where a completeness claim was falsified by the next sweep** (rev 4 → rev 5, rev 5 → rev 6, rev 6 → rev 7). The rate of new findings is falling — 6, then 3, then 4, and none in rev 7 was a blocker — but the honest position is that discovery converges asymptotically. Treat the tracker as live, not final.

**How this was verified (rev 5–6).** Rev 5 added a module-level-globals sweep. Rev 6 added the sweep flagged as missing in rev 5 — **class-level mutable attributes**: 159 exist, 156 are constant lookup tables, and only 3 are mutated at runtime. Two (`base_adapter._bound_ports`, `FlowPriceMonitor._pending`) are already lock-guarded; one, `broker/hdfcsky/api/data.py:346` `_CHART_SYMBOL_CACHE`, is an unbounded class attribute shared across every instance and mutated without a lock (`GT-A12-13`).

Rev 6 also corrected four errors found by review, listed in §10.4.

**Earlier (rev 5).** A residual sweep for module-level mutable globals across `services/`, `blueprints/`, `utils/`, `subscribers/`, `websocket_proxy/`, `sandbox/` and `database/` found **six surfaces that rev 4 had not mapped** — including `database/qty_freeze_db.py`, which carries the identical clear-then-refill defect as the symbol cache and sits on the order-splitting path. It was named in the source audit's GT-14 and lost when A12 was drafted. All six now have rows.
