...
def get_chain():
    ...
    try:
        ...
        underlying = str(data.get("underlying") or "").strip()[:20]
        exchange = str(data.get("exchange") or "").strip()[:20]
        expiry_date = str(data.get("expiry_date") or "").strip()[:10]

        try:
            strike_count = int(data.get("strike_count", 5))
            strike_count = max(1, min(strike_count, 25))
...
def get_data():
    ...
    try:
        ...
        for s in symbols:
            ...
            # Input validation for symbol and exchange
            if not re.match(r"^[A-Z0-9.\-_: ]+$", sym_str):
                return jsonify({"status": "error", "message": f"Invalid symbol format: {sym_str}"}), 400
            if not re.match(r"^[A-Z0-9_]+$", exch_str):
                return jsonify({"status": "error", "message": f"Invalid exchange format: {exch_str}"}), 400

            sanitized_symbols.append({
                "symbol": sym_str,
                "exchange": exch_str
            })
