...
def api_freeze_add():
    ...
    try:
        ...
        if not symbol or freeze_qty is None:
            return jsonify({'status': 'error', 'message': 'Symbol and freeze_qty are required'}), 400

        # Check if already exists
        existing = QtyFreeze.query.filter_by(exchange=exchange, symbol=symbol).first()
        if existing:
            return jsonify({'status': 'error', 'message': f'{symbol} already exists for {exchange}'}), 400
...
def api_freeze_delete(id):
    ...
    try:
        ...
        return jsonify({
            'status': 'success',
            'message': f'Deleted freeze qty for {symbol}'
        })
    except Exception as e:
        freeze_db_session.rollback()
...
def api_holiday_delete(id):
    ...
    try:
        ...
        return jsonify({
            'status': 'success',
            'message': f'Deleted holiday: {description}'
        })
    except Exception as e:
        calendar_db_session.rollback()
...
def api_timings_edit(exchange):
    ...
    try:
        ...
        if not start_time or not end_time:
            return jsonify({'status': 'error', 'message': 'Start time and end time are required'}), 400
        ...
        if update_market_timing(exchange, start_time, end_time):
            ...
            return jsonify({'status': 'error', 'message': f'Error updating timing for {exchange}'}), 500
