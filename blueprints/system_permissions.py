        ...
        if perms:
            ...
            if is_sensitive:
                if actual_mode != expected_mode:
                    result['issue'] = f"Permission should be {format_permission(expected_mode)}, currently {format_permission(actual_mode)}"
                else:
                    result['is_correct'] = True
            else:
                ...
                if (owner_actual & owner_expected) != owner_expected:
                    result['issue'] = f"Owner permission should be at least {oct(owner_expected)[2:]}, currently {oct(owner_actual)[2:]}"
                else:
                    result['is_correct'] = True
    ...
    try:
        ...
        return jsonify({
            ...
            'data': {
                'fixed': fixed,
                'failed': failed,
                'message': f"Fixed {len(fixed)} items, {len(failed)} failed" if fixed or failed else "No changes needed"
            }
        })
    except Exception as e:
