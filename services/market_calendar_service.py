...
def get_holidays(year: Optional[int] = None) -> Tuple[bool, Dict[str, Any], int]:
    """
...
def get_timings(date_str: str) -> Tuple[bool, Dict[str, Any], int]:
    ...
    try:
        ...
        if query_date < min_date or query_date > max_date:
            return False, {
                'status': 'error',
                'message': 'Date must be between 2020-01-01 and 2050-12-31'
            }, 400
        ...
        return False, {
            'status': 'error',
            'message': 'An error occurred while fetching market timings'
        }, 500


def check_holiday(date_str: str, exchange: Optional[str] = None) -> Tuple[bool, Dict[str, Any], int]:
    """
    ...
    try:
        ...
        return False, {
            'status': 'error',
            'message': 'An error occurred while checking holiday status'
        }, 500
