...
def execute_approved_order(pending_order_id: int) -> Tuple[bool, Dict[str, Any], int]:
    ...
    try:
        ...
        if pending_order.status != 'approved':
            logger.error(f"Cannot execute pending order {pending_order_id}: status is '{pending_order.status}', not 'approved'")
            return False, {
                'status': 'error',
        ...
        if not api_key or not auth_token or not broker:
            logger.error(f"Cannot execute pending order {pending_order_id}: missing api_key, auth_token, or broker")
        ...
        logger.info(f"Executing approved order: pending_order_id={pending_order_id}, api_type={api_type}, user={user_id}")
        ...
        try:
            ...
            if api_type == 'placeorder':
                ...
                logger.info(f"Calling split_order with api_key={api_key[:8]}..., auth_token={'present' if auth_token else 'None'}, broker={broker}")
                success, response_data, status_code = split_order(
                    split_data=order_data,
                    api_key=api_key,
                    auth_token=auth_token,
                    broker=broker
                )
                logger.info(f"Split order result: success={success}, status={status_code}, response={response_data}")

            elif api_type == 'optionsorder':
                from services.place_options_order_service import place_options_order
                logger.info(f"Calling place_options_order with api_key={api_key[:8]}..., auth_token={'present' if auth_token else 'None'}, broker={broker}")
                success, response_data, status_code = place_options_order(
                    options_data=order_data,
                    api_key=api_key,
                    auth_token=auth_token,
                    broker=broker
                )
                logger.info(f"Options order result: success={success}, status={status_code}, response={response_data}")
                ...
                return False, {
                    'status': 'error',
                    'message': f'Unknown order type: {api_type}'
                }, 400
            ...
            if success and 'orderid' in response_data:
                ...
                try:
                    ...
                    if status_success and 'data' in status_response:
                        ...
                        logger.info(f"Order executed: pending_order_id={pending_order_id}, broker_order_id={broker_order_id}, status={actual_status}")
                    else:
                        # Fallback to 'open' if status check fails
                        update_broker_status(pending_order_id, broker_order_id, 'open')
                        logger.info(f"Order executed successfully: pending_order_id={pending_order_id}, broker_order_id={broker_order_id}")
