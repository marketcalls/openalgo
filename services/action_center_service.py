    ...
    try:
        ...
        if api_type == 'optionsorder':
            ...
            order_dict.update({
                ...
                'price': order_data.get('price', '0'),
                'trigger_price': order_data.get('trigger_price', '0'),
                'price_type': order_data.get('price_type', order_data.get('pricetype', 'MARKET')),
...
def get_action_center_data(user_id: str, status_filter: str = None) -> Tuple[bool, Dict[str, Any], int]:
    """
    ...
    try:
        ...
        logger.info(f"Retrieved {len(orders_list)} orders for user {user_id} (filter: {status_filter})")
        ...
        return False, {
            'status': 'error',
            'message': f'Failed to retrieve action center data: {str(e)}'
        }, 500
