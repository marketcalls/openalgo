...
_CHART_INDICATOR_DOC = (
    Path(__file__).resolve().parents[2] / "docs" / "prompt" / "indicators" / "chart-indicators.md"
)
...
class ChartToolkit(OpenAlgoToolkit):
    ...
    def analyse_chart(self, lookback_bars: int = 0) -> str:
        ...
        slopes: list[float] = []
        rails: dict[str, Any] = {}
        for name, pivots in (("resistance", highs), ("support", lows)):
            fit = geom.fit_line(pivots)
            if fit is None:
                continue
    ...
    def draw_trendline(self, side: str = "auto", lookback_bars: int = 0, note: str = "") -> str:
        ...
        chosen as the one touching the most of the others, so it touches the bars
        it claims to touch rather than being fitted to a cloud.
        ...
        if wanted == "range":
            ...
            if band is not None:
                ...
                described.append(
                    ...
                    f"from {ist_label(band['from_time'])} to {ist_label(band['to_time'])}"
                )
            kinds = ()
    ...
    def list_chart_indicators(self, query: str = "") -> str:
        ...
        library of 127. Only 39 names exist in both, so AlphaTrend and HalfTrend
    ...
    def add_chart_indicator(self, name: str, settings: dict | None = None) -> str:
        ...
        Args:
            name: The indicator id, as listed by ``list_chart_indicators``, for
                example ``alphatrend``, ``halftrend`` or ``supertrend``.
