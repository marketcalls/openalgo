...
from services.agent.providers import litellm_kwargs, litellm_model_id, validate_provider_config
...
def resolve_model(model_id: int | str | None = None) -> ResolvedModel:
    ...
    resolved = ResolvedModel(
        ...
        supports_reasoning=_reasoning_capable(litellm_model_id(kind, model_name), row),
...
def _reasoning_capable(litellm_id: str, row: Any) -> bool:
    ...
    Args:
        ...
        row: The ``ag_provider_model`` row.

    Returns:
        True when a reasoning effort may be sent.
    """
    flag = bool(getattr(row, "supports_reasoning", False))
    try:
        import litellm
