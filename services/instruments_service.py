...
def get_instruments(exchange: str = None, api_key: str = None, format: str = 'json') -> Tuple[bool, Any, int, Dict[str, str]]:
    """
    ...
    try:
        ...
        logger.info(f"Found {len(results_data)} instruments" + (f" for exchange: {exchange}" if exchange else ""))
        ...
        if format == 'csv':
            ...
            output = StringIO()
            if results_data:
                fieldnames = ['symbol', 'brsymbol', 'name', 'exchange', 'brexchange', 'token', 'expiry', 'strike', 'lotsize', 'instrumenttype', 'tick_size']
