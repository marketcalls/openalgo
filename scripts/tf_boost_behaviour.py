...
def analyse(day: str) -> list[dict]:
    ...
    for n, (symbol, mv) in enumerate(sorted(movement.items()), start=1):
        try:
            ...
            direction = mv.get("run_direction") or ("up" if mv["rank_delta"] else "up")
            turn_min = mv.get("run_from_min")
            idx = 0
            if turn_min is not None:
                idx = min(range(len(bars)), key=lambda i: abs(_minute_of_day(bars[i]) - turn_min))
            ...
            baseline_bars = bars[: idx + 1] if idx >= OPENING_RANGE_BARS else prev_bars or bars
            ...
            rows.append(
                {
                    ...
                    "rank_at_trigger": rank_at_trigger,
...
def report(rows: list[dict]) -> None:
    ...
    print(
        f"\n{len(rows)} symbols studied. {len(catchable)} reached +1% after their turn, "
        f"{len(clean)} of those without first giving back {HEAT_LIMIT_PCT}%.\n"
    )

    by_level: dict[str, list[dict]] = {}
    for r in rows:
        by_level.setdefault(r["level_broken"] or "no level broken", []).append(r)
    print("what gave way at the turn:")
...
def main() -> None:
    ...
    print(f"Intraday Boost behaviour study -- {day}\n")
    rows = analyse(day)
    report(rows)
    if "--report" not in sys.argv and rows:
        written = upsert_behaviour(rows)
        print(
            f"\nstored {written} rows in tf_boost_behaviour (day={day}); re-running replaces them"
        )
        out_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "log")
        os.makedirs(out_dir, exist_ok=True)
        write_markdown(day, rows, os.path.join(out_dir, f"tf_boost_behaviour_{day}.md"))
