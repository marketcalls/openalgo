...
class Strategy(Base):
    ...
    platform = Column(String(50), nullable=False)  # AMIBroker/Python/Tradingview/Metatrader
    mode = Column(String(10), nullable=False)  # LONG/SHORT/BOTH
...
def create_strategy(name, webhook_id, user_id, platform, mode, is_intraday=True, start_time=None, end_time=None, squareoff_time=None):
...
def get_user_strategies(user_id):
    ...
    try:
        return Strategy.query.filter_by(user_id=user_id).all()
    except Exception as e:
...
def update_strategy_times(strategy_id, start_time=None, end_time=None, squareoff_time=None):
    ...
    try:
        ...
        logger.error(f"Error updating strategy times for {strategy_id}: {str(e)}")
        db_session.rollback()
        return False
