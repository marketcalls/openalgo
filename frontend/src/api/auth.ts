...
import type { LoginCredentials, LoginResponse, SessionInfo, BrokerInfo } from '@/types/auth';
...
export const authApi = {
   ...
   */
  initiateBrokerAuth: async (broker: string): Promise<{ redirect_url?: string; requires_totp?: boolean }> => {
   ...
   */
  submitTOTP: async (broker: string, totp: string, additionalFields?: Record<string, string>): Promise<LoginResponse> => {
    const formData = new FormData();
