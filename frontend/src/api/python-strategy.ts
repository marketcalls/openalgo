...
export const pythonStrategyApi = {
  ...
  getStrategy: async (strategyId: string): Promise<PythonStrategy> => {
    const response = await webClient.get<{ strategy: PythonStrategy }>(`/python/api/strategy/${strategyId}`);
  ...
  getStrategyContent: async (strategyId: string): Promise<PythonStrategyContent> => {
    const response = await webClient.get<PythonStrategyContent>(`/python/api/strategy/${strategyId}/content`);
   ...
   */
  uploadStrategy: async (name: string, file: File): Promise<ApiResponse<{ strategy_id: string }>> => {
    const formData = new FormData();
    formData.append('name', name);
    formData.append('file', file);

    const response = await webClient.post<ApiResponse<{ strategy_id: string }>>('/python/new', formData, {
      headers: {
   ...
   */
  exportStrategy: async (strategyId: string, version: 'saved' | 'current' = 'saved'): Promise<Blob> => {
  ...
  startStrategy: async (strategyId: string): Promise<ApiResponse<{ process_id: number }>> => {
    const response = await webClient.post<ApiResponse<{ process_id: number }>>(`/python/start/${strategyId}`);
    return response.data;
  },
   ...
   * Schedule a strategy
   */
  scheduleStrategy: async (strategyId: string, config: ScheduleConfig): Promise<ApiResponse<void>> => {
    const response = await webClient.post<ApiResponse<void>>(`/python/schedule/${strategyId}`, config);
    return response.data;
  },
   ...
   */
  saveEnvVariables: async (strategyId: string, variables: EnvironmentVariables): Promise<ApiResponse<void>> => {
  ...
  checkAndStartPending: async (): Promise<ApiResponse<{ started: number }>> => {
    const response = await webClient.post<ApiResponse<{ started: number }>>('/python/check-contracts');
    return response.data;
  },
};
