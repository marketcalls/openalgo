...
export const chartinkApi = {
  ...
  getStrategies: async (): Promise<ChartinkStrategy[]> => {
    const response = await webClient.get<{ strategies: ChartinkStrategy[] }>('/chartink/api/strategies');
   ...
   */
  getStrategy: async (strategyId: number): Promise<{ strategy: ChartinkStrategy; mappings: ChartinkSymbolMapping[] }> => {
    const response = await webClient.get<{ strategy: ChartinkStrategy; mappings: ChartinkSymbolMapping[] }>(
   ...
   */
  createStrategy: async (data: CreateChartinkStrategyRequest): Promise<ApiResponse<{ strategy_id: number }>> => {
    const response = await webClient.post<ApiResponse<{ strategy_id: number }>>('/chartink/api/strategy', data);
    return response.data;
  },
   ...
   */
  addSymbolMapping: async (strategyId: number, data: AddChartinkSymbolRequest): Promise<ApiResponse<void>> => {
   ...
   */
  addBulkSymbols: async (strategyId: number, csvData: string): Promise<ApiResponse<{ added: number; failed: number }>> => {
   ...
   */
  deleteSymbolMapping: async (strategyId: number, mappingId: number): Promise<ApiResponse<void>> => {
    const response = await webClient.post<ApiResponse<void>>(
