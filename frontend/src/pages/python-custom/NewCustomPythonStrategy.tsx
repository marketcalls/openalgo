...
export default function NewCustomPythonStrategy() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response =
        mode === 'upload'
          ? await pythonStrategyCustomApi.uploadStrategy(name, file!, {
              start_time: startTime,
              stop_time: stopTime,
              days: selectedDays,
              exchange,
            })
          : await pythonStrategyCustomApi.addStrategyFromPath(name, strategyPath.trim(), {
              start_time: startTime,
              stop_time: stopTime,
              days: selectedDays,
              exchange,
  ...
  return (
    <div className="container mx-auto py-6 max-w-2xl space-y-6">
      <Button variant="ghost" asChild>
        <Link to="/python-custom">
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to Python Strategies Custom
        </Link>
      </Button>

      <div>
        <h1 className="text-2xl font-bold tracking-tight">Add Strategy (Custom Module)</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 border rounded-lg p-4">
              ...
              <div className="space-y-2">
                ...
                {isCrypto && (
                  <p className="text-xs text-muted-foreground">
                    CRYPTO supports all 7 days by default.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2 justify-end">
              <Button type="button" variant="outline" onClick={() => navigate('/python-custom')}>
                Cancel
              </Button>
      ...
      <Collapsible open={showExample} onOpenChange={setShowExample}>
        <CollapsibleTrigger asChild>
          <Button variant="outline" className="w-full justify-between">
            ...
            <span>{showExample ? 'Hide' : 'Show'}</span>
          </Button>
        </CollapsibleTrigger>
        <CollapsibleContent className="mt-2">
          <pre className="bg-muted p-4 rounded-lg text-sm overflow-x-auto">
            <code>{EXAMPLE_STRATEGY}</code>
          </pre>
        </CollapsibleContent>
      </Collapsible>
    </div>
