import { Activity, Play, RefreshCw, Square, Trash2, Wifi, WifiOff, X } from 'lucide-react'
        ...
        case 'market_data': {
          ...
          setActiveSymbols((prev) => {
            ...
            if (mode === 1 || mode === 2) {
              ...
              if (marketData.average_price !== undefined)
                newData.average_price = marketData.average_price
              if (marketData.last_trade_quantity !== undefined)
                newData.last_trade_quantity = marketData.last_trade_quantity
              if (marketData.total_buy_quantity !== undefined)
                newData.total_buy_quantity = marketData.total_buy_quantity
              if (marketData.total_sell_quantity !== undefined)
                newData.total_sell_quantity = marketData.total_sell_quantity
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Real-time testing for multiple symbols with dynamic symbol management
          </p>
        </CardHeader>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              onClick={() => subscribeAllMode('Quote')}
              variant="outline"
              disabled={!isConnected}
            >
              Subscribe All Quote
            ...
            <Button
              onClick={() => subscribeAllMode('Depth')}
              variant="outline"
              disabled={!isConnected}
            >
              Subscribe All Depth
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-1">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => subscribe(symbolData.symbol, symbolData.exchange, 'LTP')}
                  >
                    LTP
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => subscribe(symbolData.symbol, symbolData.exchange, 'Quote')}
                  >
                    Quote
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => subscribe(symbolData.symbol, symbolData.exchange, 'Depth')}
                  >
                    Depth
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div>
                    ...
                    <span className="font-mono">
                      {symbolData.data.open !== undefined ? formatPrice(symbolData.data.open) : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">High:</span>{' '}
                    <span className="font-mono">
                      {symbolData.data.high !== undefined ? formatPrice(symbolData.data.high) : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Low:</span>{' '}
                    <span className="font-mono">
                      {symbolData.data.low !== undefined ? formatPrice(symbolData.data.low) : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Close:</span>{' '}
                    <span className="font-mono">
                      {symbolData.data.close !== undefined
                        ? formatPrice(symbolData.data.close)
                        : '-'}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono">
                      {symbolData.data.volume !== undefined
                        ? formatVolume(symbolData.data.volume)
                        : '-'}
                    </span>
                  </div>
                </div>
