    ...
    <div className="w-80 border-l border-border bg-card flex flex-col h-full">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={handleDelete} title="Delete" aria-label="Delete"><Trash2 className="h-4 w-4" /></Button>
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleClose} title="Close" aria-label="Close"><X className="h-4 w-4" /></Button>
        </div>
      </div>
        ...
        <div className="space-y-4 p-4">
          ...
          {nodeType === 'webhookTrigger' && (<>
            ...
            {webhookQuery.isLoading ? (<div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin" /></div>) : webhookQuery.data ? (<>
              <div className="space-y-2"><Label className="text-xs">Webhook URL</Label><div className="flex gap-1"><Input readOnly value={nodeData.symbol ? `${webhookQuery.data.webhook_url}/${nodeData.symbol}` : webhookQuery.data.webhook_url} className="font-mono text-[10px] h-8" /><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => copyToClipboard(nodeData.symbol ? `${webhookQuery.data.webhook_url}/${nodeData.symbol}` : webhookQuery.data.webhook_url, 'URL')} title="Copy webhook URL" aria-label="Copy webhook URL to clipboard"><Copy className="h-3 w-3" /></Button></div></div>
              <div className="space-y-2"><Label className="text-xs">Webhook Secret</Label><div className="flex gap-1"><div className="relative flex-1"><Input readOnly type={showSecret ? 'text' : 'password'} value={webhookQuery.data.webhook_secret} className="font-mono text-[10px] h-8 pr-8" /><Button variant="ghost" size="icon" className="absolute right-0 top-0 h-8 w-8" onClick={() => setShowSecret(!showSecret)} title={showSecret ? "Hide secret" : "Show secret"} aria-label={showSecret ? "Hide secret" : "Show secret"}>{showSecret ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}</Button></div><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => copyToClipboard(webhookQuery.data.webhook_secret, 'Secret')} title="Copy webhook secret" aria-label="Copy webhook secret to clipboard"><Copy className="h-3 w-3" /></Button></div></div>
