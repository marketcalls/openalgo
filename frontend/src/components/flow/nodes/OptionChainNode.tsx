...
import type { OptionChainNodeData } from '@/types/flow'

interface OptionChainNodeProps {
  data: OptionChainNodeData
  selected?: boolean
}

export const OptionChainNode = memo(({ data, selected }: OptionChainNodeProps) => {
  return (
      ...
      <div className="p-2">
        <div className="mb-1.5 flex items-center gap-1.5">
          ...
          <div>
            ...
            <div className="text-[9px] text-muted-foreground">{data.exchange || 'NSE_INDEX'}</div>
          </div>
        </div>
        <div className="space-y-0.5 rounded bg-muted/50 px-1.5 py-1">
          ...
          {data.strikeCount && (
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-muted-foreground">Strikes</span>
              <span className="mono-data text-[10px] font-medium">{data.strikeCount}</span>
            </div>
          )}
        </div>
        {data.outputVariable && (
