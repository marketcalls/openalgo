...
from database.token_db import get_br_symbol, get_token, get_symbol_info
...
def transform_data(data, client_id=None, auth_token=None):
    ...
    if str(data.get("pricetype") or "").upper() == "MARKET":
        ...
        try:
            if auth_token:
                ...
                logger.info(f"MPP Quote Response: Symbol={data['symbol']}, Exchange={data['exchange']}, "
                ...
                logger.info(f"MPP Symbol Info: InstrumentType={instrument_type}, TickSize={tick_size}")
                ...
                if ltp > 0:
                    ...
                    order_type = "LIMIT"
                    logger.info(f"MPP Conversion Complete: Symbol={data['symbol']}, OrderType=MARKET->LIMIT, "
                               f"FinalPrice={protected_price}")
                else:
                    logger.warning(f"MPP Warning: LTP is 0 or invalid for Symbol={data['symbol']}, "
                                  f"Exchange={data['exchange']}. Proceeding with regular market order")
            else:
                logger.warning(f"MPP Warning: No auth token available for Symbol={data['symbol']}. "
                              f"Cannot fetch quotes for MPP adjustment")
        except Exception as e:
            logger.error(f"MPP Error: Failed to apply MPP for Symbol={data['symbol']}, "
    ...
    logger.info(f"Transformed order data: exchange={transformed['exchange']}, "
