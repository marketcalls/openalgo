...
from broker.jainam.streaming.jainam_websocket import JainamWebSocketClient
...
from .jainam_mapping import JainamExchangeMapper, JainamCapabilityRegistry
from database.token_db import get_symbol


class JainamWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Jainam XTS specific implementation of the WebSocket adapter"""
    ...
    def __init__(self):
        ...
        self.logger.info(f"Jainam adapter initialized with ZMQ port: {self.zmq_port}")
    
    def initialize(self, broker_name: str, user_id: str, auth_data: Optional[Dict[str, str]] = None) -> None:
        """
        Initialize connection with Jainam XTS WebSocket API
        
        Args:
            broker_name: Name of the broker (always 'jainam' in this case)
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError("Missing Jainam XTS API credentials in environment variables")
        ...
        self.logger.info(f"Using API Key: {api_key[:10]}... for Jainam XTS connection")
        
        # Create Jainam WebSocket client with API credentials
    ...
    def connect(self) -> None:
        """Establish connection to Jainam XTS WebSocket"""
    ...
    def _connect_with_retry(self) -> None:
        """Connect to Jainam XTS WebSocket with retry logic"""
        while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
            try:
                self.logger.info(f"Connecting to Jainam XTS WebSocket (attempt {self.reconnect_attempts + 1})")
    ...
    def disconnect(self) -> None:
        """Disconnect from Jainam XTS WebSocket"""
        self.logger.info("*** DISCONNECT CALLED - Starting Jainam disconnect process ***")
    ...
    def cleanup_zmq(self) -> None:
        ...
        try:
            ...
            self.logger.info("Jainam WebSocket cleanup completed successfully")
        except Exception as e:
            self.logger.exception(f"Error cleaning up ZeroMQ resources: {e}")
    
    def subscribe(self, symbol: str, exchange: str, mode: int = 2, depth_level: int = 5) -> Dict[str, Any]:
        """
        Subscribe to market data with Jainam XTS specific implementation
        ...
        if mode == 3:  # Depth mode
            if not JainamCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                ...
                actual_depth = JainamCapabilityRegistry.get_fallback_depth_level(
        ...
        # Create instrument list for Jainam XTS API
        exchange_type = JainamExchangeMapper.get_exchange_type(brexchange)
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return JainamExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> Dict[str, Any]:
        ...
        # Create instrument list for Jainam XTS API
        instruments = [{
            "exchangeSegment": JainamExchangeMapper.get_exchange_type(brexchange),
    ...
    def _on_open(self, wsapp) -> None:
        ...
        self.logger.info("Connected to Jainam XTS WebSocket")
        self.connected = True
    ...
    def _on_error(self, wsapp, error) -> None:
        ...
        self.logger.error(f"Jainam XTS WebSocket error: {error}")
    
    def _on_close(self, wsapp) -> None:
        """Callback when connection is closed"""
        self.logger.info("Jainam XTS WebSocket connection closed")
        self.connected = False
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(f"RAW JAINAM DATA: Type: {type(message)}, Data: {message}")
