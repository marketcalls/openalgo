...
from broker.jainam.database.master_contract_db import SymToken, db_session
...
class BrokerData:
    def __init__(self, auth_token, feed_token=None, user_id=None):
        """Initialize Jainam data handler with authentication token"""
        self.auth_token = auth_token
        self.feed_token = feed_token
        self.user_id = user_id
        
        # Map common timeframe format to Jainam intervals
        self.timeframe_map = {
    ...
    def _fetch_market_data(self, token: dict, message_code: int) -> dict:
        """
        Helper method to fetch market data from Jainam API
        Args:
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                # Jainam expects MMM DD YYYY HHMMSS in IST
                ...
                if not response or response.get('type') != 'success':
                    ...
                    raise Exception(f"Error from Jainam API: {response.get('description', 'Unknown error')}")
