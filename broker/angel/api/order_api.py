...
def get_ltp(auth, symbols):
    """
    Get real-time LTP data for multiple symbols
    Args:
        auth: Authentication token
        symbols: List of symbols in format ["EXCHANGE:SYMBOL"]
    Returns:
        Dictionary with LTP data for each symbol
    """
    result = {"status": "success", "data": {}}
    
    try:
        # Convert symbols to Angel Broking format and get tokens
        symbol_tokens = []
        for symbol_str in symbols:
            exchange, symbol = symbol_str.split(':')
        ...
        # Prepare payload for LTP API
        payload = json.dumps({"mode": "FULL", "tradingsymbols": symbol_tokens})
        
        # Make API call to get LTP data
        ...
        if response.get('status') and response.get('data'):
            for symbol_str in symbols:
                ...
                # Find matching symbol data in response
                for symbol_data in response['data']:
                    if (symbol_data.get('tradingsymbol') == br_symbol and 
                        symbol_data.get('exchange') == exchange):
                        result['data'][symbol_str] = {
                            "ltp": float(symbol_data.get('ltp', 0))
                        }
                        break
        else:
            return {"status": "error", "message": "Failed to fetch LTP data"}
