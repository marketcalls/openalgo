...
class BrokerData:  
    ...
    def get_history_direct(self, symbol: str, exchange: str, interval: str, 
        ...
        try:
            ...
            br_symbol = get_br_symbol(symbol, exchange)
            token = get_token(symbol, exchange)
            logger.debug(f"Debug - Direct call for Symbol: {symbol}, Exchange: {exchange}")
            logger.debug(f"Debug - Broker Symbol: {br_symbol}, Token: {token}")
            logger.debug(f"Debug - Date range: {start_date} to {end_date}, Interval: {interval}")
            ...
            # Prepare payload for historical data API (single call)
            payload = {
                "exchange": exchange,
                "symboltoken": token,
            ...
            logger.debug(f"Debug - Direct API call payload: {payload}")
            ...
            response = get_api_response("/rest/secure/angelbroking/historical/v1/getCandleData",
                                      ...
                                      "POST",
                                      payload)
            logger.info(f"Debug - Direct API Response Status: {response.get('status')}")
            
            if not response.get('status'):
                error_msg = response.get('message', 'Unknown error')
                logger.info(f"Debug - Direct API Error: {error_msg}")
                raise Exception(f"Error from Angel API: {error_msg}")
            
            # Extract candle data and create DataFrame
            data = response.get('data', [])
            if not data:
                logger.debug("Debug - No data received from direct API call")
                return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            
            df = pd.DataFrame(data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            logger.debug(f"Debug - Received {len(data)} candles from direct API call")
            ...
            # Add OI column for F&O contracts (set to 0 for direct calls)
            df['oi'] = 0
            ...
            logger.error(f"Debug - Direct API Error: {str(e)}")
            raise Exception(f"Error fetching historical data directly: {str(e)}")
