...
def get_order_book(auth):
    ...
    try:
        ...
        logger.info(f"[DeltaExchange] RAW /v2/orders (open) first item: {open_result.get('result', [])[:1]}")
        ...
        logger.info(f"[DeltaExchange] RAW /v2/orders/history first item: {hist_result.get('result', [])[:1]}")
...
def get_trade_book(auth):
    ...
    try:
        ...
        logger.info(f"[DeltaExchange] RAW /v2/fills first item: {result.get('result', [])[:1]}")
        if result.get("success"):
...
def get_positions(auth):
    ...
    try:
        ...
        logger.info(f"[DeltaExchange] RAW /v2/positions/margined first item: {result.get('result', [])[:1]}")
        if result.get("success"):
    ...
    try:
        ...
        logger.info(f"[DeltaExchange] RAW /v2/wallet/balances first item: {wallet_result.get('result', [])[:1]}")
