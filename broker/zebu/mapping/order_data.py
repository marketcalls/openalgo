...
# Zebu (Noren) order statuses arrive with underscores ("TRIGGER_PENDING") and
...
def normalize_order_status(raw_status):
    """Map a Zebu status to an OpenAlgo status (open/complete/cancelled/rejected)."""
...
def map_order_data(order_data):
    ...
    if order_data:
        for order in order_data:
            ...
            # unconditionally - otherwise a lookup miss leaves raw Noren values
