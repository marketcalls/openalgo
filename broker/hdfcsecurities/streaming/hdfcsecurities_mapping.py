"""HDFC Securities InvestRight WebSocket mapping utilities: subscription types
and depth-level capabilities.

The feed (`/wsapi/v1/session`) subscribes by prefixed scripId ("NSE_2885",
"NSE_INDEX_26000") and delivers protobuf GenericDTO frames. The subscription
`type` selects how much of the packet is populated:
...
Depth is 5 levels; InvestRight publishes no deeper book. If it ever adds one,
...
class HDFCSecuritiesCapabilityRegistry:
    """HDFC Securities InvestRight WebSocket capabilities."""

    # OpenAlgo capability -> InvestRight subscription type.
    #
    ...
    # Documented feed limits: up to 1500 instruments on a single connection and
    # up to three simultaneous connections per API key.
    MAX_INSTRUMENTS_PER_CONNECTION = 1500
    ...
    def get_fallback_depth_level(cls, requested_depth_level, exchange=None):
        """InvestRight only publishes 5 levels; any request falls back to 5."""
