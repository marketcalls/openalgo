...
class AcagarwalXTSExchangeMapper:
    """Maps between OpenAlgo exchange codes and AcagarwalXTS specific exchange types"""

    # Exchange type mapping for AcagarwalXTS broker
    # Format: {OpenAlgo_Exchange: AcagarwalXTS_Exchange_Code}
    ...
    # Reverse mapping for converting AcagarwalXTS exchange codes to OpenAlgo format
    # Format: {AcagarwalXTS_Exchange_Code: OpenAlgo_Exchange}
    ...
    def get_exchange_type(exchange):
        """
        Convert OpenAlgo exchange code to AcagarwalXTS specific exchange type

        Args:
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NSEFO')

        Returns:
            int: Exchange type code for AcagarwalXTS API
        """
        if exchange is None:
        ...
        # Mapping based on AcagarwalXTS API documentation:
        # "NSECM": 1, "NSEFO": 2, "NSECD": 3, "BSECM": 11, "BSEFO": 12, "MCXFO": 51
    ...
    @staticmethod
    def get_openalgo_exchange(acagarwalxts_code):
        """
        Convert AcagarwalXTS exchange code to OpenAlgo exchange code

        Args:
            acagarwalxts_code (int): AcagarwalXTS exchange code

        Returns:
            str: OpenAlgo exchange code
        """
        return AcagarwalXTSExchangeMapper.REVERSE_EXCHANGE_TYPES.get(
            acagarwalxts_code, "NSE"
        )  # Default to NSE if not found


class AcagarwalXTSCapabilityRegistry:
    """
    Registry of AcagarwalXTS broker's capabilities including supported exchanges,
    subscription modes, and market depth levels
    """

    # AcagarwalXTS broker capabilities
