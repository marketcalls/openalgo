...
from broker.acagarwalxts.streaming.acagarwalxts_websocket import AcagarwalXTSWebSocketClient
...
from .acagarwalxts_mapping import AcagarwalXTSCapabilityRegistry, AcagarwalXTSExchangeMapper


class AcagarwalXTSWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """AcagarwalXTS specific implementation of the WebSocket adapter"""

    def __init__(self):
        super().__init__()
        self.logger = get_logger("acagarwalxts_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "acagarwalxts"
        ...
        self.logger.info(f"AcagarwalXTS adapter initialized with ZMQ port: {self.zmq_port}")

    def initialize(
        self, broker_name: str, user_id: str, auth_data: dict[str, str] | None = None
    ) -> None:
        """
        Initialize connection with AcagarwalXTS WebSocket API

        Args:
            broker_name: Name of the broker (always 'acagarwalxts' in this case)
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError("Missing AcagarwalXTS API credentials in environment variables")
        ...
        self.logger.info("Using configured API key for AcagarwalXTS connection")

        # Create AcagarwalXTS WebSocket client with API credentials
        self.ws_client = AcagarwalXTSWebSocketClient(
            api_key=api_key,
    ...
    def connect(self) -> None:
        """Establish connection to AcagarwalXTS WebSocket"""
    ...
    def _connect_with_retry(self) -> None:
        """Connect to AcagarwalXTS WebSocket with retry logic"""
        with self.lock:
        ...
        try:
            while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
                ...
                try:
                    self.logger.info(
                        f"Connecting to AcagarwalXTS WebSocket (attempt {self.reconnect_attempts + 1})"
                    )
                    client.connect()
    ...
    def disconnect(self) -> None:
        """Disconnect from AcagarwalXTS WebSocket"""
        self.logger.info("*** DISCONNECT CALLED - Starting AcagarwalXTS disconnect process ***")
    ...
    def subscribe(
        ...
        """
        Subscribe to market data with AcagarwalXTS specific implementation
        ...
        if mode == 3:  # Depth mode
            if not AcagarwalXTSCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = AcagarwalXTSCapabilityRegistry.get_fallback_depth_level(
        ...
        # Create instrument list for AcagarwalXTS API
        exchange_type = AcagarwalXTSExchangeMapper.get_exchange_type(brexchange)
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return AcagarwalXTSExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": AcagarwalXTSExchangeMapper.get_exchange_type(brexchange),
    ...
    def _on_open(self, wsapp) -> None:
        ...
        self.logger.info("Connected to AcagarwalXTS WebSocket")
        self.connected = True
    ...
    def _on_error(self, wsapp, error) -> None:
        ...
        self.logger.error(f"AcagarwalXTS WebSocket error: {error}")

    def _on_close(self, wsapp) -> None:
        """Callback when connection is closed"""
        self.logger.info("AcagarwalXTS WebSocket connection closed")
        self.connected = False
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(f"RAW ACAGARWALXTS DATA: Type: {type(message)}, Data: {message}")
            self.logger.info(
    ...
    def _process_json_data(self, data: dict):
        ...
        try:
            ...
            # Based on AcagarwalXTS API documentation:
