...
from broker.acagarwalxts.baseurl import MARKET_DATA_URL
...
def download_csv_acagarwalxts_data(output_path):
...
def process_acagarwalxts_nse_csv(path):
    """
    Processes the acagarwalxts CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing acagarwalxts NSE CSV Data")
...
def process_acagarwalxts_bse_csv(path):
    """
    Processes the acagarwalxts CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing acagarwalxts BSE CSV Data")
...
def process_acagarwalxts_nfo_csv(path):
    """
    Processes the AcagarwalXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing AcagarwalXTS NFO CSV Data")
...
def process_acagarwalxts_cds_csv(path):
    """
    Processes the acagarwalxts CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing acagarwalxts CDS CSV Data")
...
def process_acagarwalxts_bfo_csv(path):
    """
    Processes the AcagarwalXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing AcagarwalXTS BFO CSV Data")
...
def process_acagarwalxts_mcx_csv(path):
    """
    Processes the AcagarwalXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
    logger.info("Processing AcagarwalXTS MCX CSV Data")
...
def delete_acagarwalxts_temp_data(output_path):
...
def master_contract_download():
    ...
    output_path = "tmp"
    try:
        download_csv_acagarwalxts_data(output_path)
        delete_symtoken_table()

        # Process each segment with individual error handling
        try:
            token_df = process_acagarwalxts_nse_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing NSE CSV: {e}")

        try:
            token_df = process_acagarwalxts_bse_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing BSE CSV: {e}")

        try:
            token_df = process_acagarwalxts_nfo_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing NFO CSV: {e}")

        try:
            token_df = process_acagarwalxts_bfo_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
        ...
        delete_acagarwalxts_temp_data(output_path)
