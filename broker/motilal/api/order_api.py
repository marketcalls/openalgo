...
def _position_book_ok(positions_data):
    """Motilal says status SUCCESS for a read that worked.

    get_api_response rewrites every FAILURE, and every body it could not
    parse, to status "error".
    """
    if not isinstance(positions_data, dict):
        return False
    return str(positions_data.get("status", "")).upper() == "SUCCESS"
...
def _get_cached_positions(auth):
    ...
    positions_data = read_position_book("motilal", lambda: get_positions(auth), _position_book_ok)
