...
class TrustlineExchangeMapper:
    """Maps between OpenAlgo exchange codes and Trustline XTS specific exchange types"""
    
    # Exchange type mapping for Trustline XTS broker
    # Format: {OpenAlgo_Exchange: Trustline_Exchange_Code}
    # Based on Trustline API documentation:
    ...
    # Reverse mapping for converting Trustline exchange codes to OpenAlgo format
    # Format: {Trustline_Exchange_Code: OpenAlgo_Exchange}
    ...
    def get_exchange_type(exchange):
        """
        Convert OpenAlgo exchange code to Trustline XTS specific exchange type
        
        Args:
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NSEFO')
            
        Returns:
            int: Exchange type code for Trustline XTS API
        """
        if exchange is None:
        ...
        # Mapping based on Trustline API documentation:
        # "NSECM": 1, "NSEFO": 2, "NSECD": 3, "BSECM": 11, "BSEFO": 12, "MCXFO": 51
    ...
    @staticmethod
    def get_openalgo_exchange(trustline_code):
        """
        Convert Trustline XTS exchange code to OpenAlgo exchange code
        
        Args:
            trustline_code (int): Trustline exchange code
            
        Returns:
            str: OpenAlgo exchange code
        """
        return TrustlineExchangeMapper.REVERSE_EXCHANGE_TYPES.get(trustline_code, 'NSE')  # Default to NSE if not found


class TrustlineCapabilityRegistry:
    """
    Registry of Trustline broker's capabilities including supported exchanges, 
    subscription modes, and market depth levels
    """
    
    # Trustline XTS broker capabilities
