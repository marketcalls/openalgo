...
class BaseBrokerWebSocketAdapter(ABC):
    ...
    def subscribe_bulk(self, symbols, mode=2, depth_level=5):
        """
        Subscribe to multiple symbols in a single batch operation.

        This default implementation calls subscribe() for each symbol.
        Broker adapters should override this method for efficient batch subscriptions.
            ...
            dict: Response with status and per-symbol results
                  {
                      ...
                      'results': [
                          {'symbol': 'X', 'exchange': 'NSE', 'status': 'success', ...},
                          {'symbol': 'Y', 'exchange': 'NSE', 'status': 'error', 'message': '...'},
                      ],
        ...
        for symbol_info in symbols:
            ...
            response = self.subscribe(symbol, exchange, mode, depth_level)
            ...
            if response.get("status") == "success":
                results.append({
                    ...
                    "status": "success",
                    "mode": mode,
                    "depth_level": response.get("actual_depth", depth_level),
                    **{k: v for k, v in response.items() if k not in ["status", "message"]}
                })
                subscribed_count += 1
            else:
                results.append({
    ...
    def unsubscribe_bulk(self, symbols, mode=2):
        """
        Unsubscribe from multiple symbols in a single batch operation.

        This default implementation calls unsubscribe() for each symbol.
        Broker adapters should override this method for efficient batch unsubscriptions.
