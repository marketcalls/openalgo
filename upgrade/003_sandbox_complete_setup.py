...
def create_all_indexes(conn):
    ...
    conn.execute(text("CREATE INDEX IF NOT EXISTS idx_config_key ON sandbox_config(key)"))
...
def insert_default_config(conn):
    ...
    default_configs = [
        ('initial_capital', '10000000.00', 'Initial capital for new sandbox users (₹1 Crore)'),
        ...
        ('enable_partial_fills', 'false', 'Whether to enable partial order fills'),
        ('enable_holdings', 'false', 'Whether to track T+1 settlements to holdings'),
        ('margin_multiplier_cnc', '1.0', 'Margin multiplier for CNC orders'),
        ('margin_multiplier_mis', '5.0', 'Margin multiplier for MIS orders'),
        ('margin_multiplier_nrml', '3.0', 'Margin multiplier for NRML orders'),
    ]

    for key, value, description in default_configs:
        # Check if config exists
        result = conn.execute(text("SELECT 1 FROM sandbox_config WHERE key = :key"), {'key': key})
        if not result.fetchone():
            conn.execute(text("""
                INSERT INTO sandbox_config (key, value, description)
