...
- **[I-2] Dead code**: `services/telegram_bot_service_v2.py:42` and `services/telegram_bot_service_fixed.py:42` each create an `httpx.AsyncClient` but are not imported anywhere — candidates for deletion (no runtime impact). Both files have since been deleted.
