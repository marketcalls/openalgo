...
- **Node.js — frontend contributors only** - [Download Node.js](https://nodejs.org/). The `engines` field in `frontend/package.json` is the source of truth: `>=20.20.0 || >=22.22.0 || >=24.13.0`. An older 20.x, 22.x or 24.x release will refuse to install. Skip this if you are working on the backend or documentation.
...
contradiction: Git applies ignore rules only to untracked files, so the ignore entry keeps
contributors from committing a half-built local `dist/`, while the copy already tracked on
...
- The same workflow builds and pushes the Docker image to Docker Hub.

**For contributors:**
- **Do not commit `frontend/dist/` yourself.** CI owns that directory. `.gitignore` keeps a local build out of `git add` for you, and a hand-built `dist/` in a pull request is review noise that conflicts on merge.
...
- [ ] Frontend tests pass, if you changed frontend code (`cd frontend && npm test`)
