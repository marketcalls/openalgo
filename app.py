...
from blueprints.strategy_hub_bp import strategy_hub_bp  # Import the Strategy Hub blueprint
...
def create_app():
    ...
    app.register_blueprint(strategy_hub_bp)  # Register Strategy Hub blueprint
    ...
    with app.app_context():
        ...
        # Start the Strategy Hub ZMQ listener (PULL + periodic port-scan poll on
        # real OS threads). No DB dependency — pure in-memory registry — so it
        # can start immediately alongside the order-update adapters.
        try:
            from blueprints.strategy_zmq_listener import start_listener as start_strategy_hub

            start_strategy_hub()
        except Exception as e:
            logger.error(f"Failed to start Strategy Hub listener: {e}")
