...
PINE_SHORT_TITLE = "VWAP BB super Confluence 2"
PINE_VERSION = "v5"
...
DEFAULT_USE_TREND_FILTER = True  # useTrendFilter
...
# Confluence defaults.
DEFAULT_CONFLUENCE_MODE = "3: Opposite"
...
# Reversal defaults.
DEFAULT_REQUIRE_DOUBLE_TOUCH = True
DEFAULT_USE_MTF_CONFIRMATION = False
DEFAULT_MTF_TIMEFRAME = "15"
DEFAULT_INCLUDE_MEET_POINT = True
DEFAULT_LINE_LENGTH = 3

DEFAULT_SAMPLE_PATH = Path(r"D:\TV_proj\output\individual\VWAP_BB_Confluence.csv")
...
def _evaluate_confluence_row(
    mode: str,
    high_value: float,
    low_value: float,
    tolerance: float,
    upper_vw_values: Iterable[float],
    lower_vw_values: Iterable[float],
    upper_bb_values: Iterable[float],
    lower_bb_values: Iterable[float],
) -> tuple[bool, float, bool, float]:
    """Evaluate one Pine confluence row using first-match loop semantics."""
    upper_confl = False
    lower_confl = False
    meet_upper = np.nan
    meet_lower = np.nan
    ...
    if mode == "None":
        return upper_confl, meet_upper, lower_confl, meet_lower

    if mode == "1: Same Side Upper":
        for vwu in upper_vw_values:
            if np.isnan(vwu):
                continue
            for bbu in upper_bb_values:
                if np.isnan(bbu):
                    continue
                if high_value >= (vwu - tolerance) and high_value >= (bbu - tolerance):
                    upper_confl = True
                    meet_upper = (vwu + bbu) / 2.0
                    break
            if upper_confl:
                break
    elif mode == "2: Same Side Lower":
        for vwl in lower_vw_values:
            if np.isnan(vwl):
                continue
            for bbl in lower_bb_values:
                if np.isnan(bbl):
                    continue
                if low_value <= (vwl + tolerance) and low_value <= (bbl + tolerance):
                    lower_confl = True
                    meet_lower = (vwl + bbl) / 2.0
                    break
            if lower_confl:
                break
    elif mode == "3: Opposite":
        for vwu in upper_vw_values:
            if np.isnan(vwu):
                continue
            for bbl in lower_bb_values:
                if np.isnan(bbl):
                    continue
                if high_value >= (vwu - tolerance) and low_value <= (bbl + tolerance):
                    upper_confl = True
                    meet_upper = (vwu + bbl) / 2.0
                    break
            if upper_confl:
                break
        for vwl in lower_vw_values:
            ...
            for bbu in upper_bb_values:
                if np.isnan(bbu):
                    continue
                if low_value <= (vwl + tolerance) and high_value >= (bbu - tolerance):
                    lower_confl = True
                    meet_lower = (vwl + bbu) / 2.0
                    break
            if lower_confl:
                break
    else:
        raise ValueError(f"Unsupported confluence mode: {mode}")

    return upper_confl, meet_upper, lower_confl, meet_lower


def _touch_high(levels: Iterable[float], high_value: float, tolerance: float) -> bool:
    """Return whether the high touches any upper level within tolerance."""
    for level in levels:
        if np.isnan(level):
            continue
        if high_value >= (level - tolerance):
            return True
    return False


def _touch_low(levels: Iterable[float], low_value: float, tolerance: float) -> bool:
    """Return whether the low touches any lower level within tolerance."""
    for level in levels:
        if np.isnan(level):
            continue
        if low_value <= (level + tolerance):
            return True
    return False


def _timeframe_to_freq(timeframe: str) -> str:
    """Convert a TradingView-style timeframe string to a pandas resample rule."""
    tf = timeframe.strip().upper()
    if not tf:
        raise ValueError("mtf_timeframe cannot be empty.")
    if tf.isdigit():
        return f"{int(tf)}min"
    ...
    if tf.endswith("M") and tf[:-1].isdigit():
        ...
        if value >= 12:
            return f"{value}ME"
        return f"{value}min"
    raise ValueError(f"Unsupported mtf_timeframe: {timeframe}")


def _timeframe_to_timedelta(timeframe: str) -> Optional[pd.Timedelta]:
    """Best-effort conversion of a TradingView-style timeframe to a timedelta."""
    tf = timeframe.strip().upper()
    if tf.isdigit():
...
def _infer_base_interval(index: pd.DatetimeIndex) -> Optional[pd.Timedelta]:
    """Infer the typical base bar spacing from the median observed delta."""
    if len(index) < 2:
        return None
...
def _resample_ohlcv(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    """Resample OHLCV data to a higher timeframe using closed-bar semantics."""
    rule = _timeframe_to_freq(timeframe)
    agg = (
...
def _align_confirmed_htf_bool(
    base_index: pd.DatetimeIndex,
    htf_series: pd.Series,
) -> pd.Series:
    """
    Align completed higher-timeframe booleans to the base timeframe.

    This mirrors `request.security(..., lookahead_off)` on historical bars by
    exposing only the last completed higher-timeframe value.
    """
    left = pd.DataFrame({"timestamp": pd.DatetimeIndex(base_index)})
    right = (
        htf_series.astype(float)
        .rename("value")
    ...
    aligned = merged["value"].fillna(0.0).astype(bool)
    return pd.Series(aligned.to_numpy(), index=base_index, dtype=bool)
...
def calculate_indicators(
    ...
    confluence_mode: str = DEFAULT_CONFLUENCE_MODE,
    ...
    require_double_touch: bool = DEFAULT_REQUIRE_DOUBLE_TOUCH,
    use_mtf_confirmation: bool = DEFAULT_USE_MTF_CONFIRMATION,
    mtf_timeframe: str = DEFAULT_MTF_TIMEFRAME,
    include_meet_point: bool = DEFAULT_INCLUDE_MEET_POINT,
    line_length: int = DEFAULT_LINE_LENGTH,
    event_data: dict[str, set[pd.Timestamp]] | None = None,
    _compute_mtf_confirmation: bool = True,
) -> pd.DataFrame:
    """
    Replicate the VWAP/BB super confluence indicator and exported sample columns.
    ...
    - dual Bollinger sets
    - touch-based, mode-dependent confluence detection
    - optional trend filter, reversal detection, and MTF confirmation
    """
    ...
    if anchor not in SUPPORTED_ANCHORS:
        raise ValueError(f"Unsupported anchor: {anchor}")
    if confluence_mode not in SUPPORTED_CONFLUENCE_MODES:
    ...
    bar_index = pd.Series(np.arange(len(working)), index=working.index, dtype=int)
    ...
    upper_vw_arrays = np.column_stack([vwu1.to_numpy(), vwu2.to_numpy()])
    lower_vw_arrays = np.column_stack([vwl1.to_numpy(), vwl2.to_numpy()])
    upper_bb_arrays = np.column_stack(
        [bbu1a.to_numpy(), bbu1b.to_numpy(), bbu2a.to_numpy(), bbu2b.to_numpy()]
    )
    lower_bb_arrays = np.column_stack(
        [bbl1a.to_numpy(), bbl1b.to_numpy(), bbl2a.to_numpy(), bbl2b.to_numpy()]
    )
    ...
    for i in range(n):
        row_upper_confl, row_meet_upper, row_lower_confl, row_meet_lower = _evaluate_confluence_row(
            confluence_mode,
            high_values[i],
            low_values[i],
            tol_values[i],
        ...
        upper_confluence[i] = int(row_upper_confl)
        lower_confluence[i] = int(row_lower_confl)
        meet_upper[i] = row_meet_upper
        meet_lower[i] = row_meet_lower

    is_uptrend = (avgval > avgval.shift(1)).fillna(False)
    if use_trend_filter:
        upper_confluence = np.where(is_uptrend.to_numpy(dtype=bool), 0, upper_confluence)
        lower_confluence = np.where(is_uptrend.to_numpy(dtype=bool), lower_confluence, 0)

    upper_confl_series = pd.Series(upper_confluence, index=working.index, dtype=int)
    lower_confl_series = pd.Series(lower_confluence, index=working.index, dtype=int)
    meet_upper_series = pd.Series(meet_upper, index=working.index, dtype=float)
    meet_lower_series = pd.Series(meet_lower, index=working.index, dtype=float)

    if use_mtf_confirmation and _compute_mtf_confirmation:
        base_interval = _infer_base_interval(working.index)
        target_interval = _timeframe_to_timedelta(mtf_timeframe)
        if base_interval is not None and target_interval is not None and target_interval <= base_interval:
            raise ValueError(
                "mtf_timeframe must be higher than the base timeframe when MTF confirmation is enabled."
            )

        resampled = _resample_ohlcv(working, mtf_timeframe)
        mtf_calculated = calculate_indicators(
            resampled,
            symbol=symbol,
            length=length,
            ...
            use_volume_weighted_variance=use_volume_weighted_variance,
            vw_stdev_len=vw_stdev_len,
            vwap_k1=vwap_k1,
            vwap_k2=vwap_k2,
            hide_on_dwm=hide_on_dwm,
            anchor=anchor,
            ...
            show_bg_zone=show_bg_zone,
            plot_mid_line=plot_mid_line,
            confluence_mode=confluence_mode,
            bb_len1=bb_len1,
            bb_k1a=bb_k1a,
            bb_k1b=bb_k1b,
            enable_set2=enable_set2,
            bb_len2=bb_len2,
            bb_k2a=bb_k2a,
            bb_k2b=bb_k2b,
            require_double_touch=require_double_touch,
            use_mtf_confirmation=False,
            mtf_timeframe=mtf_timeframe,
            include_meet_point=include_meet_point,
            line_length=line_length,
            event_data=event_data,
            _compute_mtf_confirmation=False,
        )
        mtf_upper_confl = _align_confirmed_htf_bool(working.index, mtf_calculated["Upper_Confluence"])
        mtf_lower_confl = _align_confirmed_htf_bool(working.index, mtf_calculated["Lower_Confluence"])
    else:
        mtf_upper_confl = pd.Series(True, index=working.index, dtype=bool)
        mtf_lower_confl = pd.Series(True, index=working.index, dtype=bool)

    touch_upper_vw = np.zeros(n, dtype=bool)
    touch_upper_bb = np.zeros(n, dtype=bool)
    touch_upper = np.zeros(n, dtype=bool)
    touch_lower_vw = np.zeros(n, dtype=bool)
    touch_lower_bb = np.zeros(n, dtype=bool)
    touch_lower = np.zeros(n, dtype=bool)

    for i in range(n):
        upper_vw_touch = _touch_high(upper_vw_arrays[i], high_values[i], tol_values[i])
        upper_bb_touch = _touch_high(upper_bb_arrays[i], high_values[i], tol_values[i])
        lower_vw_touch = _touch_low(lower_vw_arrays[i], low_values[i], tol_values[i])
        lower_bb_touch = _touch_low(lower_bb_arrays[i], low_values[i], tol_values[i])

        touch_upper_vw[i] = upper_vw_touch
        touch_upper_bb[i] = upper_bb_touch
        touch_lower_vw[i] = lower_vw_touch
        touch_lower_bb[i] = lower_bb_touch

        upper_touch = (upper_vw_touch or upper_bb_touch) and (
            (not require_double_touch) or (upper_vw_touch and upper_bb_touch)
        )
        if include_meet_point and upper_confl_series.iat[i] == 1:
            meet_value = meet_upper_series.iat[i]
            if not np.isnan(meet_value) and high_values[i] >= (meet_value - tol_values[i]):
                upper_touch = True
        touch_upper[i] = upper_touch

        lower_touch = (lower_vw_touch or lower_bb_touch) and (
            (not require_double_touch) or (lower_vw_touch and lower_bb_touch)
        )
        if include_meet_point and lower_confl_series.iat[i] == 1:
            meet_value = meet_lower_series.iat[i]
            if not np.isnan(meet_value) and low_values[i] <= (meet_value + tol_values[i]):
                lower_touch = True
        touch_lower[i] = lower_touch

    touch_upper_series = pd.Series(touch_upper, index=working.index, dtype=bool)
    touch_lower_series = pd.Series(touch_lower, index=working.index, dtype=bool)
    upper_reversal = (
        touch_upper_series.shift(1).fillna(False)
        & (close.shift(1) > open_.shift(1)).fillna(False)
        & (low < pd.concat([low.shift(1), open_.shift(1)], axis=1).min(axis=1))
        & mtf_upper_confl.shift(1).fillna(False)
    )
    lower_reversal = (
        touch_lower_series.shift(1).fillna(False)
        & (close.shift(1) < open_.shift(1)).fillna(False)
        & (high > pd.concat([high.shift(1), open_.shift(1)], axis=1).max(axis=1))
        & mtf_lower_confl.shift(1).fillna(False)
    )

    is_uptrend_prev = (avgval.shift(1) > avgval.shift(2)).fillna(False)
    if use_trend_filter:
        upper_reversal = upper_reversal & is_uptrend_prev
        lower_reversal = lower_reversal & (~is_uptrend_prev)

    background_color = pd.Series(np.nan, index=working.index, dtype=float)
    if show_bg_zone:
        confl_mask = upper_confl_series.eq(1) | lower_confl_series.eq(1)
        background_color = background_color.mask(confl_mask, BACKGROUND_ZONE_ACTIVE_CODE)

    upper_confl_line_level = meet_upper_series.where(upper_confl_series.eq(1))
    lower_confl_line_level = meet_lower_series.where(lower_confl_series.eq(1))
    upper_reversal_level = pd.Series(np.nan, index=working.index, dtype=float)
    lower_reversal_level = pd.Series(np.nan, index=working.index, dtype=float)

    prior_upper_meet = meet_upper_series.shift(1)
    prior_lower_meet = meet_lower_series.shift(1)
    prior_high = high.shift(1)
    prior_low = low.shift(1)
    upper_reversal_level.loc[upper_reversal] = np.where(
        prior_upper_meet.loc[upper_reversal].notna(),
        prior_upper_meet.loc[upper_reversal],
        prior_high.loc[upper_reversal],
    )
    lower_reversal_level.loc[lower_reversal] = np.where(
        prior_lower_meet.loc[lower_reversal].notna(),
        prior_lower_meet.loc[lower_reversal],
        prior_low.loc[lower_reversal],
    )
    ...
    out = working.assign(
        ...
        raw_vwap_variance=raw_variance_series,
        tol=tol,
        is_uptrend=is_uptrend.astype(bool),
        Trendicator=avgval,
        ...
        BB2_Upper_B=bbu2b,
        BB2_Lower_B=bbl2b,
        Upper_Confluence=upper_confl_series,
        Lower_Confluence=lower_confl_series,
        Background_Color=background_color,
        meet_upper=meet_upper_series,
        meet_lower=meet_lower_series,
        mtf_upper_confl=mtf_upper_confl.astype(bool),
        mtf_lower_confl=mtf_lower_confl.astype(bool),
        touch_upper_vw=pd.Series(touch_upper_vw, index=working.index, dtype=bool),
        touch_upper_bb=pd.Series(touch_upper_bb, index=working.index, dtype=bool),
        touch_upper=pd.Series(touch_upper, index=working.index, dtype=bool),
        touch_lower_vw=pd.Series(touch_lower_vw, index=working.index, dtype=bool),
        touch_lower_bb=pd.Series(touch_lower_bb, index=working.index, dtype=bool),
        touch_lower=pd.Series(touch_lower, index=working.index, dtype=bool),
        upper_reversal=upper_reversal.astype(bool),
        lower_reversal=lower_reversal.astype(bool),
        upper_confl_line_level=upper_confl_line_level,
        lower_confl_line_level=lower_confl_line_level,
        upper_reversal_line_level=upper_reversal_level,
        lower_reversal_line_level=lower_reversal_level,
        upper_confl_line_end_bar=(bar_index + line_length - 1).where(upper_confl_series.eq(1)),
        lower_confl_line_end_bar=(bar_index + line_length - 1).where(lower_confl_series.eq(1)),
        upper_reversal_line_end_bar=(bar_index + line_length - 1).where(upper_reversal),
        lower_reversal_line_end_bar=(bar_index + line_length - 1).where(lower_reversal),
    )
    return out
...
def run_internal_sanity_checks() -> None:
    """Run deterministic unit-style checks for non-sample code paths."""
    idx = pd.date_range("2020-12-31 09:15:00", periods=48, freq="5min", tz="UTC")
    base = pd.Series(np.linspace(100.0, 110.0, len(idx)), index=idx)
    df = pd.DataFrame(
        {
            "open": base - 0.3,
            "high": base + 0.8,
    ...
    event_dates = {"Earnings": {pd.Timestamp("2020-12-31", tz="UTC")}}
    event_reset = _build_is_new_period(df.index, "Earnings", event_dates)
    assert bool(event_reset.iloc[0]) and not bool(event_reset.iloc[1])

    # Confluence mode tests.
    upper_only = _evaluate_confluence_row(
        "1: Same Side Upper",
        high_value=110.0,
        low_value=100.0,
        tolerance=0.2,
        upper_vw_values=[109.9, 120.0],
        lower_vw_values=[np.nan, np.nan],
        upper_bb_values=[109.85, 121.0],
        lower_bb_values=[90.0, 91.0],
    )
    assert upper_only[0] and not upper_only[2]

    lower_only = _evaluate_confluence_row(
        "2: Same Side Lower",
        high_value=110.0,
        low_value=100.0,
        tolerance=0.2,
        upper_vw_values=[120.0, 121.0],
        lower_vw_values=[100.1, 99.0],
        upper_bb_values=[120.0, 121.0],
        lower_bb_values=[100.05, 98.0],
    )
    assert lower_only[2] and not lower_only[0]

    opposite = _evaluate_confluence_row(
        "3: Opposite",
        high_value=110.0,
        low_value=100.0,
        tolerance=0.2,
        upper_vw_values=[109.9, 120.0],
        lower_vw_values=[100.1, 90.0],
        upper_bb_values=[110.05, 120.0],
        lower_bb_values=[100.05, 90.0],
    )
    assert opposite[0] and opposite[2]

    # MTF alignment tests.
    base_index = pd.date_range("2021-01-01 09:20:00", periods=6, freq="5min", tz="UTC")
    htf_index = pd.DatetimeIndex(
        pd.to_datetime(["2021-01-01 09:30:00Z", "2021-01-01 09:45:00Z"])
    )
    htf_values = pd.Series([True, False], index=htf_index, dtype=bool)
    aligned = _align_confirmed_htf_bool(base_index, htf_values)
    assert not bool(aligned.iloc[0]) and bool(aligned.iloc[2]) and not bool(aligned.iloc[5])
    ...
    # Non-event MTF path smoke test.
    calculated = calculate_indicators(
        df,
        use_mtf_confirmation=True,
        mtf_timeframe="15",
        confluence_mode="3: Opposite",
    )
    assert "mtf_upper_confl" in calculated.columns
    assert "upper_reversal" in calculated.columns
...
def main(argv: list[str]) -> int:
    ...
    print("Notes:")
    print("  Reversal and MTF-confirmation internals are implemented.")
    print("  They are not directly validated by this sample because the CSV does not export them.")
