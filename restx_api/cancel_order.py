...
class CancelOrder(Resource):
    ...
    def post(self):
        try:
            ...
            res, response_data = broker_module.cancel_order_api(order_data, AUTH_TOKEN)

            if res.status == 200:
                socketio.emit('cancel_order_event', {'orderid': order_data['orderid']})
                order_response_data = {'status': 'success', 'message': 'Order cancelled successfully'}
