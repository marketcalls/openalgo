        ...
        try:
            ...
            if get_analyze_mode():
                return make_response(jsonify({
                    'status': 'info',
                    'message': 'Cancel All Orders Analyzer: Implementation in progress',
                    'broker': broker,
                    'mode': 'analyze'
                }), 200)

            # Live Mode - Proceed with actual order cancellation
            ...
            try:
                res, response_data = broker_module.cancel_all_order_api(order_data, AUTH_TOKEN)
            except Exception as e:
                logger.error(f"Error in broker_module.cancel_all_order_api: {e}")
                traceback.print_exc()
                error_response = {
                    'status': 'error',
            ...
            if res.status == 200:
                socketio.emit('cancel_all_order_event', {
